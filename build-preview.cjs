...
// dans le fichier (base64). Les chemins "assets/(products|img)/xxx.jpg" dont
// le fichier existe sont remplaces par une image embarquee (data URI).
// Applique partout (donnees produits ET image du hero dans le HTML).
const MIME = { jpg: "jpeg", jpeg: "jpeg", png: "png", webp: "webp" };
function embed(str) {
  return str.replace(
    /assets\/(?:products|img)\/[^"'\s)]+\.(jpg|jpeg|png|webp)/gi,
    function (rel) {
